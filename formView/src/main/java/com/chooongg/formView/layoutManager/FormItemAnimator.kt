...
class FormItemAnimator : SimpleItemAnimator() {
    ...
    override fun runPendingAnimations() {
        ...
        if (additionsPending) {
            ...
            if (removalsPending || movesPending || changesPending) {
                ...
                val totalDelay = (removeDuration + max(
                    moveDuration.toDouble(), changeDuration.toDouble()
                )).toLong()
    ...
    override fun animateMove(
        ...
        var fromYTemp = fromY
        val view = holder.itemView
        fromXTemp += holder.itemView.translationX.toInt()
        fromYTemp += holder.itemView.translationY.toInt()
        resetAnimation(holder)
        val deltaX = toX - fromXTemp
        val deltaY = toY - fromYTemp
        ...
        mPendingMoves.add(MoveInfo(holder, fromXTemp, fromYTemp, toX, toY))
        return true
    }
    ...
    private fun animateChangeImpl(changeInfo: ChangeInfo) {
        ...
        if (view != null) {
            val valueAnimator = ValueAnimator.ofFloat(view.alpha, 0f)
            valueAnimator.interpolator = AccelerateInterpolator(2f)
            valueAnimator.duration = changeDuration
        ...
        if (newView != null) {
            val valueAnimator = ValueAnimator.ofFloat(newView.alpha, 1f)
            valueAnimator.interpolator = DecelerateInterpolator(2f)
            valueAnimator.duration = changeDuration
            valueAnimator.addUpdateListener {
                newView.alpha = it.animatedValue as Float
            }
            valueAnimator.start()
            val newViewAnimation = newView.animate().setDuration(changeDuration)
